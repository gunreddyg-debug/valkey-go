...
func Benchmark_Builder_Set(b *testing.B) {
	...
	b.Run("64B", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := builder.Set().Key(keysDynamic1000[i%1000]).Value(payload64B).Build()
			PutCompleted(cmd)
		}
	})
	...
	b.Run("64KB", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := builder.Set().Key(keysDynamic1000[i%1000]).Value(payload64KB).Build()
			PutCompleted(cmd)
		}
	})
}
...
func Benchmark_Builder_MGet(b *testing.B) {
	...
	b.ReportAllocs()
	b.ResetTimer()
	b.Run("10-keys", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := builder.Mget().Key(k10...).Build()
			PutCompleted(cmd)
		}
	})
	b.Run("100-keys", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := builder.Mget().Key(k100...).Build()
			PutCompleted(cmd)
		}
	})
	b.Run("1000-keys", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := builder.Mget().Key(k1000...).Build()
			PutCompleted(cmd)
		}
	})
}

// Benchmark_Builder_HGetAll measures hash argument building across dynamic keys (key-only build).
...
func Benchmark_ZAdd(b *testing.B) {
	...
	b.ReportAllocs()
	b.ResetTimer()
	b.Run("10-pairs", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run("100-pairs", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			z := builder.Zadd().Key(keysDynamic1000[i%1000]).ScoreMember()
			for j := 0; j < 100; j++ {
				z = z.ScoreMember(float64(j), keysDynamic1000[j%1000])
			}
			cmd := z.Build()
			PutCompleted(cmd)
		}
	})
	b.Run("1000-pairs", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			z := builder.Zadd().Key(keysDynamic1000[i%1000]).ScoreMember()
			for j := 0; j < 1000; j++ {
				z = z.ScoreMember(float64(j), keysDynamic1000[j%1000])
...
func Benchmark_Builder_ComplexArgs_ZAdd(b *testing.B) {
	builder := NewBuilder(InitSlot)
	b.ReportAllocs()
	b.ResetTimer()
	b.Run("10-pairs", func(b *testing.B) {
