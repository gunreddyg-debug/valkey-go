...
func testSETGET(t *testing.T, client Client, csc bool) {
	...
	for i := 0; i < keys*2 && !t.Failed(); i++ {
		...
		jobs <- func() {
			...
			if v, ok := kvs[key]; !((ok && val == v) || (!ok && IsValkeyNil(err))) {
	...
	for i := 0; i < keys*2 && !t.Failed(); i++ {
		...
		jobs <- func() {
			...
			n, err := s.WriteTo(buf)
			if v, ok := kvs[key]; !((ok && buf.String() == v && n == int64(buf.Len())) || (!ok && IsValkeyNil(err))) {
	...
	for i := 0; i < keys*2 && !t.Failed(); i += batch {
		...
		jobs <- func() {
			for j, resp := range client.DoMulti(ctx, commands...) {
				val, err := resp.ToString()
				if v, ok := kvs[cmdkeys[j]]; !((ok && val == v) || (!ok && IsValkeyNil(err))) {
...
func testMultiSETGETHelpers(t *testing.T, client Client, csc bool) {
	...
	validate := func(resp map[string]ValkeyMessage) {
